...
export const Navigation: React.FC<Props> = ({
  ...
  return (
    <nav className="bg-amber-400 border-b-2 border-amber-500 shadow-md sticky top-[68px] z-30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 space-y-2">
        {/* Zone Navigation Bar */}
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar scroll-smooth">
          <span className="text-xs font-black uppercase text-red-950 flex items-center gap-1 shrink-0 mr-1 bg-amber-300 px-2 py-1 rounded-md border border-amber-500/30">
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-full text-xs font-black transition-all shrink-0 border ${
              selectedZoneId === 'todos'
                ? 'bg-red-700 text-amber-300 border-red-900 shadow-md scale-105 ring-2 ring-white'
                : 'bg-white text-slate-800 border-amber-300 hover:bg-amber-100 hover:border-red-400'
            }`}
          >
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-1 border-t border-amber-500/20">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all shrink-0 ${
                selectedMunicipioId === 'todos'
                  ...
                  : 'bg-amber-300/80 hover:bg-white text-slate-900'
              }`}
            >
              Todos
            </button>
          ...
          {/* Sorting Dropdown */}
          <div className="flex items-center gap-1.5 shrink-0 bg-white/80 px-2 py-1 rounded-lg border border-amber-500/30">
