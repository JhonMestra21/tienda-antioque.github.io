...
export const CartFloatingBar: React.FC<Props> = ({ totalItems, totalCOP, onOpenCart }) => {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 w-[92%] max-w-lg">
      <button
        onClick={onOpenCart}
        className="w-full bg-slate-900 text-white p-3.5 sm:p-4 rounded-2xl shadow-2xl border-2 border-amber-400 flex items-center justify-between gap-3 hover:bg-slate-800 transition-all transform hover:-translate-y-0.5 active:translate-y-0 group"
      >
        <div className="flex items-center gap-3">
          <div className="relative bg-red-600 text-white p-2.5 rounded-xl shadow-inner">
            <ShoppingBag className="w-5 h-5 text-amber-300" />
            <span className="absolute -top-2 -right-2 bg-amber-400 text-slate-950 font-black text-xs w-5 h-5 rounded-full flex items-center justify-center border-2 border-slate-900">
              {totalItems}
            </span>
          </div>
